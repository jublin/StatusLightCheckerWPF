...
public sealed record LightPreset(string Status, string Hex, string Effect, int PeriodMs)
{
    ...
    public void Validate()
    {
        if (
            ...
            || Effect is not ("steady" or "blink" or "pulse")
            || PeriodMs is < 100 or > 60000
            || Hex.Length != 7
            || Hex[0] != '#'
            || !Hex.AsSpan(1).ToString().All(char.IsAsciiHexDigit)
        )
            throw new ArgumentException(
                "Choose a status, #RRGGBB color, effect, and period from 100 to 60000 ms."
            );
    }
...
public sealed record LightSettings(
    int Brightness,
    int TimeoutSeconds,
    IReadOnlyList<LightPreset> Presets
)
{
    public string[] Commands()
    {
        if (
            Brightness is < 0 or > 255
            || TimeoutSeconds is < 0 or > 86400
            || Presets.Count != 5
            || !Presets.Select(p => p.Status).Order().SequenceEqual(LightPreset.Statuses.Order())
        )
            throw new ArgumentException("Invalid light configuration.");
        return
        [
            .. Presets.Select(p => p.Command),
    ...
    public static LightSettings Parse(IReadOnlyList<string> response)
    {
        ...
        foreach (var line in response)
        {
            ...
            if (words.Length < 2)
                throw new InvalidDataException("Incomplete device reply.");
            var key = words[0] == "PRESET" ? $"PRESET {words[1]}" : words[0];
            if (!fields.TryAdd(key, words))
                throw new InvalidDataException("Duplicate device field.");
        }
        ...
        if (Field("INFO", 2)[1] != "1")
            throw new InvalidDataException("This firmware protocol version is not supported.");
        var presets = new List<LightPreset>();
        foreach (var status in LightPreset.Statuses)
        {
            var words = Field($"PRESET {status}", 7);
            var hex =
                $"#{Number(words[2], 255):X2}{Number(words[3], 255):X2}{Number(words[4], 255):X2}";
            presets.Add(new(status, hex, words[5], Number(words[6], 60000)));
        }
        var result = new LightSettings(
            Number(Field("BRIGHTNESS", 2)[1], 255),
            Number(Field("TIMEOUT", 2)[1], 86400),
            presets.AsReadOnly()
        );
        try
        {
        ...
        {
            throw new InvalidDataException("Invalid device settings.", ex);
        }
        return result;
    }
}
